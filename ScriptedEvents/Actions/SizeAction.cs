namespace ScriptedEvents.Actions
{
    ...
    public class SizeAction : IScriptAction, IHelpInfo
    {
        ...
        public ActionResponse Execute(Script script)
        {
            ...
            if (!VariableSystem.TryParse(Arguments.ElementAt(1), out float x))
                return new(MessageType.NotANumber, this, "X", Arguments.ElementAt(1));

            if (!VariableSystem.TryParse(Arguments.ElementAt(2), out float y))
                return new(MessageType.NotANumber, this, "Y", Arguments.ElementAt(2));

            if (!VariableSystem.TryParse(Arguments.ElementAt(3), out float z))
